#XGBoost + LightGBM + Stacking + Meta-Features
import numpy as np
...
# XGBoost
xg_model = xgb.XGBRegressor(n_estimators=300, learning_rate=0.05, max_depth=4, random_state=42)
xg_model.fit(x_train_scaled, y_train_scaled.ravel())
joblib.dump(xg_model, "models/xgboost.pkl")
xgb_pred = xg_model.predict(x_test_scaled)

# ========== LightGBM Quantile Regression ==========
def train_quantile_model(alpha):
    model = lgb.LGBMRegressor(objective='quantile', alpha=alpha, n_estimators=300, max_depth=4, learning_rate=0.05)
    model.fit(x_train_scaled, y_train.ravel())
    return model

q_models = {}
for q in [0.1, 0.5, 0.9]:
    model = train_quantile_model(q)
    q_models[q] = model
    joblib.dump(model, f"models_quantile/quantile_model_{int(q*100)}.pkl")

q10 = q_models[0.1].predict(x_test_scaled)
q50 = q_models[0.5].predict(x_test_scaled)
q90 = q_models[0.9].predict(x_test_scaled)
...
band_width = (q90 - q10).reshape(-1, 1)
meta_features = np.hstack([
    knn_pred.reshape(-1, 1),
    ridge_pred.reshape(-1, 1),
    xgb_pred.reshape(-1, 1),
    q50.reshape(-1, 1),     # Median
    band_width              # Q90 - Q10 width
])
...
joblib.dump(meta_model, "models/meta_learner_stacked_xgb.pkl")

# ========== Evaluation ==========
rmse = np.sqrt(mean_squared_error(y_test_orig, meta_pred))
r2 = r2_score(y_test_orig, meta_pred)
print(f"🔧 Final Stacked Ensemble RMSE: {rmse:.2f}")
print(f"✅ Final Stacked Ensemble R²: {r2:.3f}")

# ========== Save Outputs ==========
np.savetxt("predictions/stacked_xgb_predictions.csv", meta_pred, delimiter=",")
...
plt.scatter(y_test_orig, meta_pred, alpha=0.6, label="Stacked Ensemble (XGB) Prediction")
plt.plot([y_test_orig.min(), y_test_orig.max()], [y_test_orig.min(), y_test_orig.max()], 'r--', label="Ideal")
plt.xlabel("Actual")
plt.ylabel("Predicted")
plt.title("Stacked Ensemble (XGB) Predictions vs Actual")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("predictions/stacked_ensemble_xgb_predictions.pdf")
plt.close()
...
plt.title("Quantile Regression Bands (LightGBM)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("predictions/quantile_prediction_bands_xgb.pdf")
plt.close()