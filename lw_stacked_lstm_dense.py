...
# Load data
df = pd.read_csv('lw_training.csv', header=0)
...
# Define BiLSTM model with dense layers
model = Sequential()
model.add(LSTM(64, activation='relu', input_shape=(1, 11), return_sequences=True))
...
model_save = ModelCheckpoint('lw_model.h5', save_best_only=True, monitor='val_loss', mode='min')
...
np.savetxt('lw_y_test.csv', y_test, delimiter=',')
np.savetxt('lw_y_pred.csv', y_pred, delimiter=',')
